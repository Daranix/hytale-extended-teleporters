...
public class ExtendedTeleporterInteraction
extends SimpleBlockInteraction
{
    @Nonnull
    public static final BuilderCodec<ExtendedTeleporterInteraction> CODEC;
    @Nullable
    private String particle;

    static {
        CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(ExtendedTeleporterInteraction.class, ExtendedTeleporterInteraction::new, SimpleBlockInteraction.CODEC).appendInherited(new KeyedCodec("Particle", (Codec)Codec.STRING), (interaction, s) -> interaction.particle = s, interaction -> interaction.particle, (interaction, parent) -> interaction.particle = parent.particle).documentation("The particle to play on the entity when teleporting.").add()).build();
    }
    ...
    protected void interactWithBlock(@Nonnull World world, @Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull InteractionType type, @Nonnull InteractionContext context, @Nullable ItemStack itemInHand, @Nonnull Vector3i targetBlock, @Nonnull CooldownHandler cooldownHandler) {
        ...
        return;
        BlockModule.BlockStateInfo blockStateInfoComponent = (BlockModule.BlockStateInfo)blockRef.getStore().getComponent(blockRef, BlockModule.BlockStateInfo.getComponentType());
        ...
        if (info != null)
        {
            ...
            if (!info.isSelfDestruct() && playerUuid != null && !info.canPlayerUse(playerUuid) && !manager.isInBypassMode(playerUuid)) {
                return;
            }
        }
        ...
        if ((warpName == null || warpName.isEmpty()) && info != null) {
            ...
            if (infoDestination != null && !infoDestination.isEmpty()) {
                ...
                world.execute(() -> {
                    ...
                    BlockModule.BlockStateInfo blockStateInfo = (BlockModule.BlockStateInfo)blockRef.getStore().getComponent(blockRef, BlockModule.BlockStateInfo.getComponentType());
        ...
        boolean hasValidDestination = isValidDestination(teleporter, warpName);
        ...
        return;
        Teleport teleportComponent = toTeleportWithHiddenWarps(teleporter, warpName, transformComponent.getPosition(), transformComponent.getRotation(), targetBlock, world, playerUuid);
        ...
        if (teleportComponent != null) {
            ...
            if (this.particle != null) {
                ...
                ObjectList<Ref<EntityStore>> results = (ObjectList<Ref<EntityStore>>)(List)SpatialResource.getThreadLocalReferenceList();
                playerSpatialResource.getSpatialStructure().collect(particlePosition, 75.0D, (List)results);
                ParticleUtil.spawnParticleEffect(this.particle, particlePosition, (List)results, (ComponentAccessor)commandBuffer);
            }


            if (info != null && info.isSingleUse()) {
                handleSingleUseTeleporter(world, info, teleporter, blockRef, chunkRef);
            }
        }
    }


    private void handleSingleUseTeleporter(@Nonnull World world, @Nonnull TeleporterInfo info, @Nonnull Teleporter teleporter, @Nonnull Ref<ChunkStore> blockRef, @Nonnull Ref<ChunkStore> chunkRef) {
        TeleporterManager manager = TeleporterManager.getInstance();
    ...
    private boolean isValidDestination(Teleporter teleporter, @Nullable String warpName) {
        ...
        if (teleporter.getTransform() != null) {
            if (teleporter.getWorldUuid() != null) {
                return (Universe.get().getWorld(teleporter.getWorldUuid()) != null);
            }
            return true;
        }
    ...
    @Nullable
    private Teleport toTeleportWithHiddenWarps(Teleporter teleporter, @Nullable String warpName, @Nonnull Vector3d currentPosition, @Nonnull Vector3f currentRotation, @Nonnull Vector3i blockPosition, @Nonnull World currentWorld, @Nullable UUID playerUuid) {
        if (warpName != null && !warpName.isEmpty()) {
